    ...
    <div className="bg-[var(--novar-blue)] text-white">
      ...
      {[
        ...
        <section key={index} className="py-20 px-6 text-center">
          <h2 className="text-4xl font-semibold text-[var(--novar-yellow)]">{section.title}</h2>
          <p className="mt-4 max-w-3xl mx-auto">{section.text}</p>
          <a href={section.link} className="mt-6 inline-block bg-[var(--novar-yellow)] hover:brightness-110 px-6 py-3 text-lg rounded-lg text-white font-bold">
            {section.button}
          </a>
        </section>
      ))}
